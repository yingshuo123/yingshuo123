...
void MainWindow::exportDataToDatabase(const QByteArray& data)
{
    // 打开数据库
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName("E:/data.db");
    if (!db.open()) {
        printDebugInfo("无法打开数据库");
        return;
    }

    // 创建表
    QSqlQuery query;
    query.exec("CREATE TABLE IF NOT EXISTS data_table (id INTEGER PRIMARY KEY AUTOINCREMENT, data BLOB)");

    // 插入数据
    query.prepare("INSERT INTO data_table (data) VALUES (:data)");
    query.bindValue(":data", data);
    if (!query.exec()) {
        printDebugInfo("插入数据失败");
    }

    // 只保留最后三十条数据
    query.exec("DELETE FROM data_table WHERE id NOT IN (SELECT id FROM data_table ORDER BY id DESC LIMIT 30)");
